...
function Field({label,hint,children}){
  return(
    <div style={{marginBottom:14}}>
      ...
      {hint&&<div style={{fontSize:".67rem",color:"#6b6860",marginBottom:4,lineHeight:1.4}}>{hint}</div>}
      {children}
    </div>
  );
}
...
function Metric({label,value,sub,dark}){
  return(
    <div style={{background:dark?"#0f1117":"#fff",border:`1px solid ${dark?"#0f1117":"#e2e0d8"}`,borderRadius:5,padding:"12px 12px 10px"}}>
      <div style={{fontSize:".6rem",fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",color:dark?"#888":"#6b6860",marginBottom:4}}>{label}</div>
      <div style={{fontFamily:"monospace",fontSize:"1rem",fontWeight:700,color:dark?"#fff":"#0f1117"}}>{value}</div>
      {sub&&<div style={{fontSize:".65rem",color:dark?"#888":"#6b6860",marginTop:2}}>{sub}</div>}
    </div>
  );
}
...
function WfRow({label,amount,type,total}){
  ...
  return(
    <div style={{display:"flex",alignItems:"center",padding:"8px 14px",borderBottom:"1px solid #e2e0d8",background:total?"#f7f6f2":"#fff",fontWeight:total?700:400}}>
      <span style={{flex:1,fontSize:".76rem",color:"#0f1117"}}>{label}</span>
      <span style={{fontFamily:"monospace",fontSize:total?".88rem":".78rem",fontWeight:600,color}}>{amount}</span>
    </div>
  );
}

const STRATEGIES=[
  {id:"bridge",label:"🏗 Bridge",sub:"Bridging → refurb → refi"},
  {id:"cash",label:"💵 Cash",sub:"Cash → refurb → refi"},
  {id:"mortgage",label:"🏦 Mortgage",sub:"Direct BTL mortgage"},
];
...
export default function DealAnalyser({initialInputs,dealId,onSaved}){
  ...
  const handleExtracted=useCallback((s)=>{
    setInputs(prev=>({...prev,
      ...(s.address&&{address:s.address}),
      ...(s.bid&&{bid:s.bid}),
      ...(s.gdv&&{gdv:s.gdv}),
      ...(s.refurb&&{refurb:s.refurb}),
      ...(s.contingency&&{contingency:s.contingency}),
      ...(s.auctionFees&&{auctionFees:s.auctionFees}),
      ...(s.rent&&{rent:s.rent}),
    }));
    setSaved(false);
  },[]);
  ...
  const vc=r.verdict==="DEAL"?"#1a7a3c":r.verdict==="MARGINAL"?"#c47d00":"#b81c1c";
  const vb=r.verdict==="DEAL"?"#eaf5ee":r.verdict==="MARGINAL"?"#fff8e6":"#fdeaea";
  const vi=r.verdict==="DEAL"?"✅":r.verdict==="MARGINAL"?"⚠️":"🚫";
  const refiTotal=(inputs.refiArrangeFee||0)+(inputs.refiValuation||0)+(inputs.refiLegal||0)+(inputs.refiBroker||0);

  return(
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <style>{`
        .da-grid{display:grid;grid-template-columns:400px 1fr;}
        .da-sidebar{background:#fff;border-right:1px solid #e2e0d8;padding:20px 18px;overflow-y:auto;max-height:calc(100vh - 52px);}
        .da-results{padding:20px;background:#f7f6f2;overflow-y:auto;display:flex;flex-direction:column;gap:14px;}
        .da-metrics{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;}
        .da-refi{display:grid;grid-template-columns:1fr 1fr;gap:12px;}
        .da-yields{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;}
        .da-strategy{display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:16px;}
        @media(max-width:768px){
          .da-grid{grid-template-columns:1fr;}
          .da-sidebar{max-height:none;border-right:none;border-bottom:1px solid #e2e0d8;}
          .da-metrics{grid-template-columns:1fr 1fr;}
          .da-refi{grid-template-columns:1fr;}
          .da-yields{grid-template-columns:1fr 1fr;}
        }
      `}</style>
      ...
      <div className="da-grid">
        ...
        <div className="da-sidebar">
          ...
          <SL>Purchase Strategy</SL>
          <div className="da-strategy">
            {STRATEGIES.map(st=>(
              <button key={st.id} onClick={()=>set("strategy")(st.id)}
                ...
                <div style={{fontSize:".8rem",fontWeight:700,color:s===st.id?"#c8410a":"#0f1117"}}>{st.label}</div>
                <div style={{fontSize:".6rem",color:"#6b6860",marginTop:2,lineHeight:1.3}}>{st.sub}</div>
              </button>
            ))}
          </div>
          ...
          <button onClick={handleSave} disabled={saving||saved}
            style={{width:"100%",padding:"10px",background:saved?"#e2e0d8":"#c8410a",color:saved?"#888":"#fff",border:"none",borderRadius:4,fontWeight:700,fontSize:".84rem",cursor:saving||saved?"default":"pointer",marginBottom:4}}>
          ...
          <Field label="Your Bid">
            ...
            <input type="range" min={10000} max={200000} step={1000} value={inputs.bid}
              onChange={e=>set("bid")(+e.target.value)}
              style={{width:"100%",accentColor:"#c8410a",marginTop:6}}/>
          ...
          <Field label="GDV (After-Refurb Value)" hint="What will the property be worth once fully refurbished? Check sold prices on Rightmove for similar done-up properties nearby.">
          ...
          {s==="bridge"&&<>
            ...
            <div style={{background:"#f7f6f2",border:"1px solid #e2e0d8",borderRadius:4,padding:"10px 10px 4px",marginBottom:14}}>
              <div style={{fontSize:".67rem",fontWeight:700,color:"#6b6860",letterSpacing:".08em",textTransform:"uppercase",marginBottom:8}}>Refi Fees — Total: {fmt(refiTotal)}</div>
              <Field label="Arrangement Fee"><NumInput prefix="£" value={inputs.refiArrangeFee||0} onChange={set("refiArrangeFee")} step={100}/></Field>
              <Field label="Valuation Fee"><NumInput prefix="£" value={inputs.refiValuation||0} onChange={set("refiValuation")} step={50}/></Field>
              <Field label="Refi Solicitor Fees" hint="Conveyancing cost for the refinance mortgage"><NumInput prefix="£" value={inputs.refiLegal||0} onChange={set("refiLegal")} step={100}/></Field>
              <Field label="Broker Fee"><NumInput prefix="£" value={inputs.refiBroker||0} onChange={set("refiBroker")} step={100}/></Field>
            </div>
          </>}

          {/* ── CASH ── */}
          {s==="cash"&&<>
            <SL>Refinance (Exit)</SL>
            <div style={{background:"#eaf5ee",border:"1px solid #1a7a3c",borderRadius:4,padding:"8px 10px",fontSize:".72rem",color:"#1a4a2a",marginBottom:12}}>💡 No finance costs during purchase. Refinance after refurb to recycle cash.</div>
            <Field label="Refinance LTV"><NumInput suffix="%" value={inputs.refiLTV} onChange={set("refiLTV")} min={50} max={80} step={5}/></Field>
            <Field label="BTL Rate (annual)"><NumInput suffix="%" value={inputs.refiRate} onChange={set("refiRate")} min={3} max={10} step={0.1}/></Field>
            <div style={{background:"#f7f6f2",border:"1px solid #e2e0d8",borderRadius:4,padding:"10px 10px 4px",marginBottom:14}}>
              <div style={{fontSize:".67rem",fontWeight:700,color:"#6b6860",letterSpacing:".08em",textTransform:"uppercase",marginBottom:8}}>Refi Fees — Total: {fmt(refiTotal)}</div>
          ...
          {s==="mortgage"&&<>
            <SL>BTL Mortgage</SL>
            <div style={{background:"#fff8e6",border:"1px solid #c47d00",borderRadius:4,padding:"8px 10px",fontSize:".72rem",color:"#6b4400",marginBottom:12}}>⚠️ Requires habitable property. Not suitable for heavy refurb or auction.</div>
          ...
          <SL>Purchase Costs</SL>
          <div style={{background:"#f7f6f2",border:"1px solid #e2e0d8",borderRadius:4,padding:"10px 12px 10px",marginBottom:14}}>
            <div style={{fontSize:".67rem",fontWeight:700,color:"#6b6860",letterSpacing:".08em",textTransform:"uppercase",marginBottom:6}}>SDLT (Stamp Duty) — Auto-calculated</div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
              <div style={{fontSize:".7rem",color:"#6b6860",lineHeight:1.5}}>
                Banded Ltd Co rates:<br/>
                5% up to £125k · 7% on £125k–£250k · 10% above
              </div>
              <div style={{fontFamily:"monospace",fontSize:"1.1rem",fontWeight:700,color:"#0f1117"}}>{fmt(r.sdltAmt)}</div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <span style={{fontSize:".67rem",color:"#aaa"}}>Override (leave at 5 for auto):</span>
          ...
          <Field label="Purchase Solicitor Fees" hint="Conveyancing cost to buy the property">
          ...
          <Field label="Annual Running Costs" hint="Management fees, insurance, maintenance, voids"><NumInput prefix="£" value={inputs.runCosts} onChange={set("runCosts")} step={100}/></Field>

          <SL>Target</SL>
          <Field label="Min Profit Target (% of GDV)" hint="20% is standard. Higher = safer but fewer deals qualify.">
        ...
        <div className="da-results">
          {/* Verdict */}
          <div style={{background:vb,border:`1.5px solid ${vc}`,borderRadius:6,padding:"16px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div>
              <div style={{fontSize:"1.05rem",fontWeight:800,color:vc}}>{vi} {r.verdict}</div>
              <div style={{fontSize:".74rem",color:"#6b6860",marginTop:3}}>
                Margin {pct(r.marginPct)} · Target {inputs.targetMargin}% · <span style={{fontWeight:600}}>{s==="bridge"?"Bridge → Refi":s==="cash"?"Cash → Refi":"Direct Mortgage"}</span>
              </div>
            </div>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:".6rem",letterSpacing:".08em",textTransform:"uppercase",color:"#6b6860"}}>Max Bid ({inputs.targetMargin}% margin)</div>
              <div style={{fontFamily:"monospace",fontSize:"1.4rem",fontWeight:700,color:vc}}>{fmt(r.maxBid)}</div>
            </div>
          </div>
          ...
          <div className="da-metrics">
            ...
            <Metric label="Cash Left In" value={fmt(r.cashLeftIn)} sub={s==="mortgage"?"no refi":"after refi"}/>
            <Metric label="Finance Cost" value={fmt(r.finCost)} sub="total"/>
          </div>

          {/* Waterfall */}
          <div style={{background:"#fff",border:"1px solid #e2e0d8",borderRadius:5,overflow:"hidden"}}>
            <div style={{padding:"10px 14px",background:"#0f1117",fontSize:".67rem",fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",color:"#fff"}}>Cost Waterfall</div>
            ...
            <WfRow label="SDLT (Stamp Duty)" type="cost" amount={fmt(r.sdltAmt)}/>
          ...
          {/* Refi + P&L */}
          <div className="da-refi">
            <div style={{background:"#fff",border:"1px solid #e2e0d8",borderRadius:5,overflow:"hidden"}}>
              <div style={{padding:"10px 14px",background:"#0f1117",fontSize:".67rem",fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",color:"#fff"}}>{s==="mortgage"?"Mortgage":"Refinance Exit"}</div>
              <WfRow label="GDV" type="income" amount={fmt(inputs.gdv)}/>
              <WfRow label={`Mortgage (${s==="mortgage"?inputs.mortLTV:inputs.refiLTV}% LTV)`} type="cost" amount={fmt(r.mortgage)}/>
            ...
            <div style={{background:"#fff",border:"1px solid #e2e0d8",borderRadius:5,overflow:"hidden"}}>
              <div style={{padding:"10px 14px",background:"#0f1117",fontSize:".67rem",fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",color:"#fff"}}>Annual Rental P&amp;L</div>
          ...
          <div className="da-yields">
            {[["Gross Yield",pct(r.grossYield),r.grossYield],["Net Yield",pct(r.netYield),r.netYield],["Cash-on-Cash",pct(r.cocReturn),r.cocReturn]].map(([label,val,raw])=>(
              <div key={label} style={{background:"#fff",border:"1px solid #e2e0d8",borderRadius:5,padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:".76rem",color:"#6b6860"}}>{label}</span>
                <span style={{fontFamily:"monospace",fontSize:".95rem",fontWeight:700,color:yc(raw)}}>{val}</span>
              </div>
            ))}
          </div>

          {/* Cash note */}
          <div style={{background:"#fdf0ea",border:"1px solid #e8b49a",borderRadius:5,padding:"11px 14px",fontSize:".75rem",color:"#6b2e0a",lineHeight:1.5}}>
            {r.cashLeftIn<0
              ?`💰 Refi fully recycles your cash and returns ${fmt(Math.abs(r.cashLeftIn))} extra — zero cash locked in.`
              :r.cashLeftIn<20000
              ?`💰 Only ${fmt(r.cashLeftIn)} stays locked in after ${s==="mortgage"?"purchase":"refinance"} — excellent capital efficiency.`
