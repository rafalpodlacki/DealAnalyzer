export function calcDeal(inputs) {
  ...
  let finCost=0, cashDay1=0, cashLeftIn=0, mortgage=0, annualMortInt=0, monthlyMort=0, totalCost=0, maxBidFixed=0, maxBidCoeff=1;
  let arrange=0, bridgeInt=0, bridgeLoan=0;

  if (strategy === "bridge") {
    bridgeLoan     = bid * (bridgeLTV / 100);
    arrange        = bridgeLoan * (arrangeFee / 100);
    bridgeInt      = (bridgeLoan + arrange) * (bridgeRate / 100) * bridgeTerm;
    ...
    maxBidCoeff    = 1 + (sdlt/100) + (bridgeLTV/100)*(arrangeFee/100) + (bridgeLTV/100)*(1+arrangeFee/100)*(bridgeRate/100)*bridgeTerm;
  ...
  let verdict = marginPct >= targetMargin ? "DEAL" : marginPct >= targetMargin - 5 ? "MARGINAL" : "NO DEAL";

  return { sdltAmt, refurbTotal, finCost, totalCost, cashDay1, mortgage, annualMortInt, monthlyMort, cashLeftIn, grossRent, netAnnual, equityCreated, marginPct, grossYield, netYield, cocReturn, maxBid, verdict, bridgeLoan, arrange, bridgeInt };
}

export const DEFAULT_INPUTS = {
  strategy:"bridge", address:"", bid:55000, gdv:130000, auctionFees:2500,
  refurb:30000, contingency:15,
  bridgeLTV:70, bridgeRate:0.85, arrangeFee:2, bridgeTerm:9, bridgeLegal:2500,
  mortLTV:75, mortRate:5.5, mortFees:2000,
  refiLTV:75, refiRate:5.5, refiFees:5000,
  sdlt:5, legalFees:1500, rent:750, runCosts:2500, targetMargin:20,
};
